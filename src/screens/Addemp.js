...
export default function AddEditEmployee({ navigation, route }) {
  ...
  useEffect(() => {
    if (existingEmployee) {
      ...
      setPhone(existingEmployee.phone);
      setEmail(existingEmployee.email);
      setPassword(existingEmployee.password || '');
      setAddress(existingEmployee.address);
      setNotes(existingEmployee.notes);
    }
  }, [existingEmployee]);
  ...
  const handleSave = async () => {
    ...
    if (!empId || !name || !role || !department || !joinDate || !email || !password) {
      Alert.alert('Error', 'Please fill all mandatory fields');
      return;
    }
    ...
    try {
      ...
      // 2️⃣ Save employee data in Firestore
      const employeeData = {
        empId,
        name,
        role,
      ...
      Alert.alert('Success', `Employee ${existingEmployee ? 'updated' : 'added'} successfully`);
      navigation.goBack();
    } catch (error) {
      console.error(error);
